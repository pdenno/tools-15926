...
(defun read-base-templates (model)
  ...
  (with-open-file (csv (lpath :data "15926/part7/emerson/emerson-csv/base-template.csv") :direction :input)
...
(defun read-specialized-individual-templates (model)
  ...
  (with-open-file (csv (lpath :data "15926/part7/emerson/emerson-csv/specialized-individual-template.csv") :direction :input)
    (read-line csv nil :eof)
...
(defun read-class-template-instance (&key pathname msg-type start-column)
  ...
  (flet ((c2l (str) (intern (cl-ppcre:regex-replace-all " " (string-trim '(#\Space) str) "-"))))
    (with-open-file (csv (lpath :data "15926/part7/emerson/emerson-csv/class-template-instance.csv") :direction :input)
