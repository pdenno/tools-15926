;;; Automatically created by pop-gen at 2010-10-17 20:07:21.
...
(def-mm-constraint "association_ends" |Association|
   "Association ends of associations with more than two ends must be owned
    ...
    :operation-body
   "if memberEnd->size() > 2 then ownedEnd->includesAll(memberEnd)")    :original-body
...
(def-mm-constraint "binary_associations" |Association|
   "Only binary associations can be aggregations."
    ...
    :operation-body
   "self.memberEnd->exists(isComposite) implies self.memberEnd->size() = 2")    :original-body
    "true"
...
(def-mm-constraint "specialized_end_types" |Association|
   "When an association specializes another association, every end of the specific
    ...
    :operation-body
   "Sequence{1..self.memberEnd->size()}->  forAll(i | self.general->select(oclIsKindOf(Association)).oclAsType(Association)->   forAll(ga |self.memberEnd->at(i).type.conformsTo(ga.memberEnd->at(i).type)))")    :original-body
    "true"
...
;;; ====================== Classifier
;;; =========================================================
(def-mm-class |Classifier| (:CMOF) (|Namespace| |Type|)
...
(def-mm-constraint "value_specification_boolean" |Constraint|
   "The value specification for a constraint must evaluate to a Boolean value."
    ...
    :operation-body
   "self.specification().booleanValue().isOclKindOf(Boolean)")    :original-body
    "true"
...
(def-mm-constraint "lower_ge_0" |MultiplicityElement|
   "The lower bound must be a non-negative integer literal."
    ...
    :operation-body
   "lowerBound()->notEmpty() implies lowerBound() >= 0")    :original-body
...
(def-mm-constraint "has_qualified_name" |NamedElement|
   "When there is a name, and all of the containing namespaces have a name,
    ...
    :operation-body
   "(self.name->notEmpty() and self.allNamespaces()->select(ns | ns.name->isEmpty())->isEmpty()) implies   self.qualifiedName = self.allNamespaces()->iterate( ns : Namespace; result: String = self.name | ns.name->union(self.separator())->union(result))")    :original-body
    "true"
...
(def-mm-constraint "visibility_needs_ownership" |NamedElement|
   "If a NamedElement is not owned by a Namespace, it does not have a visibility."
    ...
    :editor-note "visibility has default value #public, making this fail whenever namespace is
     not specified."
    :operation-body
   "namespace->isEmpty() implies visibility->isEmpty()")    :original-body
    "true"
...
(def-mm-constraint "elements_public_or_private" |Package|
   "If an element that is owned by a package has visibility, it is public or
    ...
    :operation-body
   "self.ownedElements->forAll(e | e.visibility->notEmpty() implies e.visbility = #public or e.visibility = #private)")    :original-body
    "true"
...
;;; ====================== Parameter
;;; =========================================================
(def-mm-class |Parameter| (:CMOF) (|TypedElement| |MultiplicityElement|)
...
(def-mm-constraint "redefined_property_inherited" |Property|
   "A redefined property must be inherited from a more general classifier containing
    ...
    :operation-body
   "if (redefinedProperty->notEmpty()) then   (redefinitionContext->notEmpty() and       redefinedProperty->forAll(rp|         ((redefinitionContext->collect(fc|           fc.allParents()))->asSet())->collect(c| c.allFeatures())->asSet()->includes(rp))")    :original-body
    "true"
...
(def-mm-constraint "subsetting_context_conforms" |Property|
   "Subsetting may only occur when the context of the subsetting property conforms
    ...
    :operation-body
   "self.subsettedProperty->notEmpty() implies   (self.subsettingContext()->notEmpty() and self.subsettingContext()->forAll (sc |     self.subsettedProperty->forAll(sp |       sp.subsettingContext()->exists(c | sc.conformsTo(c)))))")    :original-body
    "true"
...
;;; ====================== StructuralFeature
;;; =========================================================
(def-mm-class |StructuralFeature| (:CMOF) (|TypedElement| |Feature| |MultiplicityElement|)
...
;;; ====================== ValueSpecification
;;; =========================================================
(def-mm-class |ValueSpecification| (:CMOF) (|TypedElement| |PackageableElement|)
