...
(defmethod print-object ((obj |LiteralString|) stream)
  (with-slots ((val |value|) (id mofi:|debug-id|)) obj
    (format stream "<uml22:LiteralString ~A, id=~A>" (or val "OclVoid") id)))

(defmethod print-object ((obj |LiteralInteger|) stream)
  (with-slots ((val |value|) (id mofi:|debug-id|)) obj 
    (format stream "<uml22:LiteralInteger ~A, id=~A>" (or val "OclVoid") id)))

(defmethod print-object ((obj |LiteralBoolean|) stream)
  (with-slots ((val |value|) (id mofi:|debug-id|)) obj
    (format stream "<uml22:LiteralBoolean ~A, id=~A>" (or val "OclVoid") id)))

(defmethod print-object ((obj |LiteralUnlimitedNatural|) stream)
  (with-slots ((val |value|) (id mofi:|debug-id|)) obj
    (format stream "<uml22:LiteralUnlimitedNatural ~A, id=~A>" (or val "OclVoid") id)))
