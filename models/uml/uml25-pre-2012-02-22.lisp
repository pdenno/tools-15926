;;; Automatically created by pop-gen at 2012-02-17 16:04:31.
;;; Source file is uml25-2012-02-17.xmi
...
(def-mm-operation "directionIsIn" |Gate| 
   ""
   :operation-body
   "true"
   :parameters
...
(def-mm-operation "directionIsOut" |Gate| 
   ""
   :operation-body
   "true"
   :parameters
...
(def-mm-operation "matches" |Gate| 
   "This query returns true if the name of this Gate matches the name of the
    in parameter Gate, and the messages for the two Gates correspond.<span>
     </span>The Message for one Gate (say A) corresponds to the Message for
    another Gate (say B) if (A and B have the same name value) and (if A is
    a sendEvent then B is a receiveEvent) <span> </span>and (if A is a receiveEvent
    then B is a sendEvent) and (A and B have the same messageSort value)"
   :operation-body
   "self.getName() = gateToMatch.getName() and  self.message.messageSort = gateToMatch.message.messageSort and self.message.name = gateToMatch.message.name and self.message.sendEvent->includes(self) implies gateToMatch.message.receiveEvent->includes(gateToMatch)  and self.message.receiveEvent->includes(self) implies gateToMatch.message.sendEvent->includes(gateToMatch)  "
   :parameters
   (list (make-instance 'ocl-parameter :parameter-name NIL :parameter-type '|Boolean|
                        :parameter-return-p T)
...
(def-mm-constraint "in_and_out" |Parameter| 
   ...
   :operation-body
   "(effect = ParameterEffectKind::delete implies (direction = ParameterDirectionKind::_in or direction = ParameterDirectionKind::inout)) and (effect = ParameterEffectKind::create implies (direction = ParameterDirectionKind::out or direction = ParameterDirectionKind::inout or direction = ParameterDirectionKind::return))")

(def-mm-constraint "not_exception" |Parameter| 
   "An input Parameter cannot be an exception."
   :operation-body
   "isException implies (direction <> ParameterDirectionKind::_in and direction <> ParameterDirectionKind::inout)")
...
(def-mm-constraint "input" |ParameterSet| 
   "If a Behavior has input Parameters that are in a ParameterSet, then any
    ...
    Parameters."
   :operation-body
   "(parameter->exists(direction = ParameterDirectionKind::_in and behavior <> null) implies    parameter->any(true).behavior.ownedParameter->forAll(p | p.parameterSet->isEmpty() implies p.direction = ParameterDirectionKind::_in))     and (parameter->exists(direction = ParameterDirectionKind::out and behavior <> null) implies      parameter->any(true).behavior.ownedParameter->forAll(p | p.parameterSet->isEmpty() implies p.direction = ParameterDirectionKind::out))")
...
(def-mm-constraint "transitions_incoming" |Pseudostate| 
   ...
   "--(kind = PseudostateKind::join) implies 
    ...
    -- This logic appears to bear little resemblance to what the description says  true          ")
...
(def-mm-constraint "initial_transition" |Transition| 
   ...
   "--self.source.oclIsKindOf(Pseudostate) implies 
    ...
    -- 4) the \"create\" stereotype in the standard profile, which should be capitalized, does not apply to Trigger; it applies to BehavioralFeature or Classifier  true")
